...
import TablesDashboard from '../components/shared/Dashboard-Table';
...
export async function fetchData() {
    try {
      const response = await fetch('https://my-json-server.typicode.com/pk2601/dashboard/posts');
      if (!response.ok) {
...
describe.skip('TablesDashboard component', () => {
    ...
    beforeEach(() => {
      render(<TablesDashboard datadash={mockData} />);
    });
    ...
    it('column sorting', async () => {
      ...
      const statusColumnHeader = screen.getByTestId('status');
      fireEvent.click(statusColumnHeader);
      const sortedRowsStatus = screen.getAllByRole('row');
      const sortedDataByStatus = mockData.slice().sort((a, b) => a.status.localeCompare(b.status));
      sortedRowsStatus.slice(1).forEach((row, index) => {
        const cells = row.querySelectorAll('td');
        expect(cells[5]).toHaveTextContent(sortedDataByStatus[index].status.toString());
      });
